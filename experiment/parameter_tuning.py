...
def run_optimization():
    ...
    for data_type in DATASETS:
        ...
        for model_name in MODELS:
            ...
            # phase 2
            # study_comice = optuna.create_study(direction='maximize', sampler=optuna.samplers.TPESampler(seed=42))
            # study_comice.optimize(
            #     lambda trial: objective_CoMICE(trial, model_name, train, valid, info, best_base_params),
            #     n_trials=n_trials_comice
            # )
            # best_base_params.update(study_comice.best_params)
            # --- 保存最终 JSON ---
