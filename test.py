...
class CoMICERecommend(BaseRecommender):
    ...
    def fit(self, train_data: pd.DataFrame, valid_data: pd.DataFrame = None, patience: int = 10):
        ...
        if valid_data is not None:
            val_filled = self.imputers[0].transform(valid_data.copy())
    ...
    def get_proba(self, test_data: pd.DataFrame):
        ...
        self.model.eval()
        all_view_probs = []
        for idx, imputer in enumerate(self.imputers):
            ...
            with torch.no_grad():
                for x, _ in loader:
                    ...
                    current_view_probs.append(probs)
            all_view_probs.append(np.concatenate(current_view_probs, axis=0))
        final_probs = np.mean(all_view_probs, axis=0)
